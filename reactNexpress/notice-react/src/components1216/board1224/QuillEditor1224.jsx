...
const QuillEditor1224 = ({ value, handleContent, quillRef, files, handleFiles }) => {
  const imageHandler = useCallback(() => {
    // 이미지 파일이 있을 때 호출
    // 이미지를 url로 바꾸기 위해서 express서버의 uploads 폴더로 전달할 폼데이터 만들기
    const formData = new FormData()

    // input태그를 DOM API를 활용하여 동적으로 생성하기
    ...
    input.onchange = async () => {
      try {
        ...
        // 폼 데이터를 express 서버에 넘겨서 이미지 위치의 URL 받아오기
        ...
        */
        ...
        console.error("이미지 업로드중에 오류 발생", error)
  ...
  const modules = useMemo(() => ({
    toolbar: { // 툴바에 넣을 기능들ㅇ르 순서대로 나열하면 된다.
      container: [
