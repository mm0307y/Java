...
public class ReBoardController0106 {
  ...
  // @query - select b_no, b_title, b_writer, b_date, b_count from react_board
 ...
 * 2) requset : 요청이 유지되는 동안에는 유지된다. - forward
 * 오라클 연동은 java로 하지만 응답 페이지는 jsp(html)에서 나간다.
 * java -> jsp 유지되려면 requset나 session을 사용해야 한다.
 ...
 * 쿠키와 세션은 시간동안에 유지가 가능하다. -> request가 필요 없이 무조건 유지한다.
 */